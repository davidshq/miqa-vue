<template>
  ...
  export default {
    ...
    setup() {
      ...
      watchEffect(() => {
        console.group('Running watchEffect');
        const res = unref(coneResolution);
        const rep = unref(representation);
        console.debug('context.value', context.value);
        if (context.value) {
          const { actor, coneSource, renderWindow } = context.value;
          console.debug('coneSource', coneSource);
      ...
      onMounted(() => {
        ...
        if (!context.value) {
            console.debug('vtkContainer.value', vtkContainer.value);
      ...
      onBeforeUnmount(() => {
        console.log('Running onBeforeUnmount');
        if (context.value) {
